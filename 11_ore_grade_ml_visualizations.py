...
def create_main_spatial_prediction_plot(plot: bool = False):
    ...
    gp_model = GaussianProcessRegressor(
        kernel=kernel_gp, alpha=0.01, n_restarts_optimizer=3
    )
    ...
    if plot:
        ...
        for ax, title, pred in zip(axes, titles, predictions):
            # Contour plot
            contour = ax.contourf(
                grid_X, grid_Y, pred, levels=15, cmap="gray", alpha=0.8
            )
            # Overlay drill holes
            ax.scatter(
                X_train[:, 0],
        ...
        # Add colorbar
        cbar = fig.colorbar(
            contour, ax=axes, orientation="horizontal", pad=0.08, aspect=40, shrink=0.8
        )
...
def create_model_comparison_plot(plot: bool = False):
    ...
    kriging_r2 = 1 - np.sum((y_test - kriging_pred) ** 2) / np.sum(
    ...
    gp_r2 = 1 - np.sum((y_test - gp_pred) ** 2) / np.sum(
        (y_test - np.mean(y_test)) ** 2
    )
    ...
    gbt_r2 = 1 - np.sum((y_test - gbt_pred) ** 2) / np.sum(
        (y_test - np.mean(y_test)) ** 2
    )
    # Create figure
    if plot:
        ...
        ax1.set_title(
            "Prediction Error", fontsize=12, fontweight="bold", loc="left", pad=20
        )
        ...
        ax2.set_title(
            "Predictive Performance", fontsize=12, fontweight="bold", loc="left", pad=20
        )
        ax2.set_ylim(0, 1.1)
