...
def calculate_experimental_variogram(data, max_distance=500, n_bins=20):
    ...
    for i in range(n_bins):
        ...
        binned_semivariance.append(
            semivariances[mask].mean() if mask.sum() > 0 else np.nan
        )
...
def build_gp_grade_model(training_data):
    ...
    kernel = ConstantKernel(1.0, (0.1, 10.0)) * RBF(1.0, (0.1, 5.0)) + WhiteKernel(
        0.1, (0.01, 1.0)
    )
...
def estimate_block_model(drillhole_data, gp_model, block_size=25):
    ...
    mean_pred, std_pred = gp_model["model"].predict(
        block_coords_normalized, return_std=True
    )
...
def main():
    ...
    logger.info(
        f"   Grade range: {drillholes['au_ppm'].min():.3f} to {drillholes['au_ppm'].max():.2f} ppm Au"
    )
    ...
    average_ore_grade = (
        total_contained_gold / total_ore_tonnes if total_ore_tonnes > 0 else 0
    )
