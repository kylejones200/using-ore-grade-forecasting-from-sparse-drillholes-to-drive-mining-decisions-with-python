...
def create_main_visualization(plot: bool = False):
    ...
    if plot:
        ...
        scatter = ax1.scatter(
            ...
            cmap="Greys",
            vmin=0,
            vmax=drillholes["au_ppm"].quantile(0.95),
            edgecolors="black",
            linewidths=0.5,
        )

        # Add high-grade highlights (hollow circles)
        high_grade = drillholes[
            drillholes["au_ppm"] > drillholes["au_ppm"].quantile(0.90)
        ]
        ax1.scatter(
            high_grade["x"],
            high_grade["y"],
        ...
        ax1.set_title(
            "Drillhole Spatial Distribution", fontsize=12, fontweight="bold", loc="left"
        )
        ...
        bin_centers, binned_semivar, bin_counts = calculate_experimental_variogram(
            drillholes
        )
        ...
        if len(valid_semivar) >= 3:
            ...
            range_param = valid_distances[
                np.argmin(np.abs(valid_semivar - 0.95 * sill))
            ]
            ...
            ax2.plot(
                ...
                "k-",
                linewidth=1.5,
                label=f"Spherical Model (range={range_param:.0f}m)",
            )
            ...
            ax2.axvline(
                x=range_param, color="gray", linestyle="-.", linewidth=0.8, alpha=0.5
            )
        ...
        ax2.set_title(
            "Experimental Variogram", fontsize=12, fontweight="bold", loc="left"
        )
        ...
        ax3.axvline(
            ...
            linestyle="--",
            linewidth=1.5,
            label=f"Mean = {mean_grade:.2f} ppm",
        )
        ax3.axvline(
            x=median_grade,
            color="gray",
            linestyle="-.",
            linewidth=1.5,
            label=f"Median = {median_grade:.2f} ppm",
        )
...
def create_accuracy_visualization(plot: bool = False):
    ...
    if plot:
        ...
        ax1.plot(
            [0, max_val], [0, max_val], "k--", linewidth=1.5, label="Perfect Prediction"
        )

        # Calculate R²
        r2 = 1 - np.sum((actual_grades - predicted_grades) ** 2) / np.sum(
            (actual_grades - actual_grades.mean()) ** 2
        )
        ...
        ax1.set_title(
            "Cross-Validation: Actual vs Predicted Grades",
            fontsize=12,
            fontweight="bold",
            loc="left",
        )
        ...
        # Add zero line
        ax2.axvline(
            x=0, color="black", linestyle="--", linewidth=1.5, label="Zero Residual"
        )
